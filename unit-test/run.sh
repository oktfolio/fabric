...
serial_packages=(
    "github.com/oktfolio/hyperledger-fabric-gm/gossip"
)
...
packages_diff() {
    git -C "${GOPATH}/src/github.com/hyperledger/fabric" diff --no-commit-id --name-only -r "${1:-HEAD}" |
        ...
        sed 's%/[^/]*$%/%' | sort -u | \
        awk '{print "github.com/oktfolio/hyperledger-fabric-gm/"$1"..."}'
}
...
main() {
    ...
    if [ x${arch} == xppc64le -o x${arch} == xs390x ]; then
        excluded_packages+=("github.com/oktfolio/hyperledger-fabric-gm/core/chaincode/platforms/java")
    fi
